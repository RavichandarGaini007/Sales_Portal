    ...
    const getTabData = () => {    
      switch (activeTab) {
        case "1":
          ...
          return desgVal ? tabData[0]?.data?.filter((item) => item.desg === desgVal) : tabData[0]?.data || [];
        case "2":
          return desgVal ? tabData[1]?.data?.filter((item) => item.desg === desgVal) : tabData[1]?.data || [];
          //return tabData[1]?.data;
        case "3":
          //return  || [];
          return tabData[2]?.data ? desgVal ? tabData[2]?.data.data?.filter((item) => item.desg === desgVal) : tabData[2]?.data.data : tabData[2]?.data;
        default:
          return [];
      }
    };
      ...
      <Card className="card-stats">
        <CardHeader>
          <div className="d-flex justify-content-between">
            ...
            <select id="roleSelect" style={{ maxWidth: "200px" }} className="form-control" value={desgVal} onChange={onDropdownClick}>
          ...
          <ModalBody>
            {selectedData ? (
              <div>
                ...
                <p><strong>Net Amount:</strong> {selectedData.net_amount}</p>
                <p><strong>Target:</strong> {selectedData.target}</p>
                <p><strong>Achievement:</strong> {selectedData.achv}%</p>
              </div>
            ) : (
